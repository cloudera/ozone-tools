...
class UnifiedRemoteOzoneAnalyzer:
    ...
    def analyze_all_oms(self):
        ...
        def group_by_key(operations):
            ...
            for op in operations:
                # Use full_key if available, otherwise fall back to key
                key = op.get('full_key', op['key'])
    ...
    def _analyze_group_inconsistent_only(self, key: str, group: Dict, analysis: Dict) -> str:
        ...
        # Extract volume and bucket from any operation in the group
        volume = None
        bucket = None
        for op in group['ops']:
            if 'volume' in op and op['volume']:
                volume = op['volume']
            if 'bucket' in op and op['bucket']:
                bucket = op['bucket']
        ...
        # Handle cases with same results (consistent)
        if len(unique_results) == 1:
            if om_count < 3:
                # Insufficient data with same results - add to insufficient_problematic
                group_info['missing_oms'] = [om for om in ['OM1', 'OM2', 'OM3'] if om not in om_results]
                result = list(unique_results)[0]
                group_info['issue'] = f"Insufficient data with consistent {result} result across {om_count} OM(s)"
                analysis['insufficient_problematic'].append(group_info)
                return 'processed'
            else:
                # Consistent success or failure with all 3 OMs - not interested
                return 'skipped'
        
        # We have different results - inconsistent behavior
        success_oms = [om for om, r in om_results.items() if r == 'SUCCESS']
        failure_oms = [om for om, r in om_results.items() if r == 'FAILURE']
        
        if om_count < 3:
            # Insufficient data with different results - now goes to inconsistent
            group_info['missing_oms'] = [om for om in ['OM1', 'OM2', 'OM3'] if om not in om_results]
            group_info['issue'] = f"Inconsistent results with insufficient data: Succeeded on {success_oms} but failed on {failure_oms}"
        ...
        # Track OMs in inconsistent operations for both cases
    ...
    def _write_human_summary(self, report: Dict, filepath: str):
        with open(filepath, 'w') as f:
            ...
            inconsistent_count = cross_summary.get('inconsistent_count', 0)
            
            f.write("\n\nOMs WITHOUT FAILURES IN INCONSISTENT OPERATIONS:\n")
            f.write("-" * 40 + "\n")
            
            if inconsistent_count == 0:
                f.write("No inconsistent operations detected.\n")
                f.write("All analyzed OMs are fully consistent:\n")
                for om in report['analysis_metadata']['oms_analyzed']:
                    f.write(f"  - {om}\n")
            elif oms_without_failures:
    ...
    def _print_key_findings(self, report: Dict):
        ...
        if inconsistent_count == 0:
            print(f"\n✅ No inconsistent operations detected.")
            print("All analyzed OMs are fully consistent:")
            for om in report['analysis_metadata']['oms_analyzed']:
                print(f"   - {om}")
        elif oms_without_failures:
