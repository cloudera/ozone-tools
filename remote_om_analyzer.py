...
# Patterns for extracting volume and bucket
VOLUME_PATTERN = re.compile(rb'volume=([^,}]+)')
BUCKET_PATTERN = re.compile(rb'bucket=([^,}]+)')
        ...
        with open(filepath, 'rb') as f:
            while not shutdown_requested:
                ...
                for line in lines:
                    ...
                    volume_match = VOLUME_PATTERN.search(params)
                    if volume_match:
                        volume = volume_match.group(1).decode('utf-8', errors='ignore').strip()
                    
                    bucket_match = BUCKET_PATTERN.search(params)
                    if bucket_match:
                        bucket = bucket_match.group(1).decode('utf-8', errors='ignore').strip()
                            
                    if key:
                        # Create full key path
                        full_key = key
                        if volume and bucket:
                            full_key = f"{volume}/{bucket}/{key}"
